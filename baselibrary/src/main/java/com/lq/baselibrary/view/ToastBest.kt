...
object ToastBest {
    ...
    /**
     * 系统用法 顺序执行 请使用 showToast（）
     */
    private var mToast: Toast? = null

//    注意 加  .show()
//    ToastBest.info(mActivity, "这是一个提示信息的Toast.").show();


    //默认情况： 给定icon，短时长，给定底色

    //一般提示 灰色
    @CheckResult
    @JvmStatic
    fun normal(context: Context, message: String): Toast? {
//        val icon =  AppApplication.getInstance().resources.getDrawable(R.mipmap.ic_clover)
        val icon =  context.resources.getDrawable(R.mipmap.ic_clover)
        return normal(context, message, Toast.LENGTH_SHORT, icon, true)
    }
    //一般提示 灰色 不带icon
    //与系统的区别，不会顺序显示执行，新的会取代旧的
    @CheckResult
    @JvmStatic
    fun normalNoIcon(context: Context, message: String): Toast? {
