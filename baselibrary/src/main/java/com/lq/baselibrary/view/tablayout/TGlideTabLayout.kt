...
class TGlideTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : HorizontalScrollView(context, attrs, defStyleAttr), ViewPager.OnPageChangeListener {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_TGlideTabLayout)
        mIndicatorStyle = ta.getInt(R.styleable.base_TGlideTabLayout_base_indicator_style, STYLE_NORMAL)
        mIndicatorColor = ta.getColor(R.styleable.base_TGlideTabLayout_base_indicator_color, Color.parseColor(if (mIndicatorStyle == STYLE_BLOCK) "#4B6A87" else "#ffffff"))
        mIndicatorHeight = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_height,
                dp2px(if (mIndicatorStyle == STYLE_TRIANGLE) 4f else (if (mIndicatorStyle == STYLE_BLOCK) -1 else 2).toFloat()).toFloat())
        mIndicatorWidth = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_width, dp2px(if (mIndicatorStyle == STYLE_TRIANGLE) 10f else -1.toFloat()).toFloat())
        mIndicatorCornerRadius = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_corner_radius, dp2px(if (mIndicatorStyle == STYLE_BLOCK) -1f else 0.toFloat()).toFloat())
        indicatorMarginLeft = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_margin_left, dp2px(0f).toFloat())
        indicatorMarginTop = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_margin_top, dp2px(if (mIndicatorStyle == STYLE_BLOCK) 7f else 0.toFloat()).toFloat())
        indicatorMarginRight = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_margin_right, dp2px(0f).toFloat())
        indicatorMarginBottom = ta.getDimension(R.styleable.base_TGlideTabLayout_base_indicator_margin_bottom, dp2px(if (mIndicatorStyle == STYLE_BLOCK) 7f else 0.toFloat()).toFloat())
        mIndicatorGravity = ta.getInt(R.styleable.base_TGlideTabLayout_base_indicator_gravity, Gravity.BOTTOM)
        mIndicatorWidthEqualTitle = ta.getBoolean(R.styleable.base_TGlideTabLayout_base_indicator_width_equal_title, false)
        mUnderlineColor = ta.getColor(R.styleable.base_TGlideTabLayout_base_underline_color, Color.parseColor("#ffffff"))
        mUnderlineHeight = ta.getDimension(R.styleable.base_TGlideTabLayout_base_underline_height, dp2px(0f).toFloat())
        mUnderlineGravity = ta.getInt(R.styleable.base_TGlideTabLayout_base_underline_gravity, Gravity.BOTTOM)
        mDividerColor = ta.getColor(R.styleable.base_TGlideTabLayout_base_divider_color, Color.parseColor("#ffffff"))
        mDividerWidth = ta.getDimension(R.styleable.base_TGlideTabLayout_base_divider_width, dp2px(0f).toFloat())
        mDividerPadding = ta.getDimension(R.styleable.base_TGlideTabLayout_base_divider_padding, dp2px(12f).toFloat())
        mTextsize = ta.getDimension(R.styleable.base_TGlideTabLayout_base_titlesize, sp2px(14f).toFloat())
        mTextSelectColor = ta.getColor(R.styleable.base_TGlideTabLayout_base_textSelectColor, Color.parseColor("#ffffff"))
        mTextUnselectColor = ta.getColor(R.styleable.base_TGlideTabLayout_base_textUnSelectColor, Color.parseColor("#AAffffff"))
        mTextBold = ta.getInt(R.styleable.base_TGlideTabLayout_base_textBold, TEXT_BOLD_NONE)
        mTextAllCaps = ta.getBoolean(R.styleable.base_TGlideTabLayout_base_textAllCaps, false)
        mTabSpaceEqual = ta.getBoolean(R.styleable.base_TGlideTabLayout_base_tab_space_equal, false)
        mTabWidth = ta.getDimension(R.styleable.base_TGlideTabLayout_base_tab_width, dp2px(-1f).toFloat())
        mTabPadding = ta.getDimension(R.styleable.base_TGlideTabLayout_base_tab_padding, if (mTabSpaceEqual || mTabWidth > 0) dp2px(0f).toFloat() else dp2px(20f).toFloat())
        ta.recycle()
    }
    ...
    fun addNewTab(title: String) {
        val tabView = View.inflate(mContext, R.layout.base_layout_tab, null)
        if (mTitles != null) {
            mTitles!!.add(title)
        }
