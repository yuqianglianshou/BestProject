...
class TSectionTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr), ValueAnimator.AnimatorUpdateListener {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_TSectionTabLayout)
        mIndicatorColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_indicator_color, Color.parseColor("#222831"))
        mIndicatorHeight = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_height, -1f)
        mIndicatorCornerRadius = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_corner_radius, -1f)
        indicatorMarginLeft = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_margin_left, dp2px(0f).toFloat())
        indicatorMarginTop = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_margin_top, 0f)
        indicatorMarginRight = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_margin_right, dp2px(0f).toFloat())
        indicatorMarginBottom = ta.getDimension(R.styleable.base_TSectionTabLayout_base_indicator_margin_bottom, 0f)
        isIndicatorAnimEnable = ta.getBoolean(R.styleable.base_TSectionTabLayout_base_indicator_anim_enable, false)
        isIndicatorBounceEnable = ta.getBoolean(R.styleable.base_TSectionTabLayout_base_indicator_bounce_enable, true)
        indicatorAnimDuration = ta.getInt(R.styleable.base_TSectionTabLayout_base_indicator_anim_duration, -1).toLong()
        mDividerColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_divider_color, mIndicatorColor)
        mDividerWidth = ta.getDimension(R.styleable.base_TSectionTabLayout_base_divider_width, dp2px(1f).toFloat())
        mDividerPadding = ta.getDimension(R.styleable.base_TSectionTabLayout_base_divider_padding, 0f)
        mTextsize = ta.getDimension(R.styleable.base_TSectionTabLayout_base_titlesize, sp2px(13f).toFloat())
        mTextSelectColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_textSelectColor, Color.parseColor("#ffffff"))
        mTextUnselectColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_textUnSelectColor, mIndicatorColor)
        mTextBold = ta.getInt(R.styleable.base_TSectionTabLayout_base_textBold, TEXT_BOLD_NONE)
        mTextAllCaps = ta.getBoolean(R.styleable.base_TSectionTabLayout_base_textAllCaps, false)
        mTabSpaceEqual = ta.getBoolean(R.styleable.base_TSectionTabLayout_base_tab_space_equal, true)
        mTabWidth = ta.getDimension(R.styleable.base_TSectionTabLayout_base_tab_width, dp2px(-1f).toFloat())
        mTabPadding = ta.getDimension(R.styleable.base_TSectionTabLayout_base_tab_padding, if (mTabSpaceEqual || mTabWidth > 0) dp2px(0f).toFloat() else dp2px(10f).toFloat())
        mBarColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_bar_color, Color.TRANSPARENT)
        mBarStrokeColor = ta.getColor(R.styleable.base_TSectionTabLayout_base_bar_stroke_color, mIndicatorColor)
        mBarStrokeWidth = ta.getDimension(R.styleable.base_TSectionTabLayout_base_bar_stroke_width, dp2px(1f).toFloat())
        ta.recycle()
    }
    ...
    fun notifyDataSetChanged() {
        ...
        for (i in 0 until tabCount) {
            tabView = View.inflate(mContext, R.layout.base_layout_tab_segment, null)
            tabView.tag = i
            addTab(i, tabView)
        }
        updateTabStyles()
