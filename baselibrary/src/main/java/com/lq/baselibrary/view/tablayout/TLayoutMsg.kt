...
class TLayoutMsg @JvmOverloads constructor(private val mContext: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : TextView(mContext, attrs, defStyleAttr) {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_MsgView)
        backgroundColor = ta.getColor(R.styleable.base_MsgView_base_mv_backgroundColor, Color.TRANSPARENT)
        cornerRadius = ta.getDimensionPixelSize(R.styleable.base_MsgView_base_mv_cornerRadius, 0)
        strokeWidth = ta.getDimensionPixelSize(R.styleable.base_MsgView_base_mv_strokeWidth, 0)
        strokeColor = ta.getColor(R.styleable.base_MsgView_base_mv_strokeColor, Color.TRANSPARENT)
        isRadiusHalfHeight = ta.getBoolean(R.styleable.base_MsgView_base_mv_isRadiusHalfHeight, false)
        isWidthHeightEqual = ta.getBoolean(R.styleable.base_MsgView_base_mv_isWidthHeightEqual, false)
        ta.recycle()
    }
