package com.lq.baselibrary.view.tablayout
...
import com.lq.baselibrary.view.tablayout.tool.TLayoutMsgTool.show
import java.util.*
...
class TTabLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr), ValueAnimator.AnimatorUpdateListener {
    ...
    private fun obtainAttributes(context: Context, attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.base_TTabLayout)
        mIndicatorStyle = ta.getInt(R.styleable.base_TTabLayout_base_indicator_style, 0)
        mIndicatorColor = ta.getColor(R.styleable.base_TTabLayout_base_indicator_color, Color.parseColor(if (mIndicatorStyle == STYLE_BLOCK) "#4B6A87" else "#ffffff"))
        mIndicatorHeight = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_height, dp2px(if (mIndicatorStyle == STYLE_TRIANGLE) 4f else (if (mIndicatorStyle == STYLE_BLOCK) -1 else 2).toFloat()).toFloat())
        mIndicatorWidth = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_width, dp2px(if (mIndicatorStyle == STYLE_TRIANGLE) 10f else -1.toFloat()).toFloat())
        mIndicatorCornerRadius = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_corner_radius, dp2px(if (mIndicatorStyle == STYLE_BLOCK) -1f else 0.toFloat()).toFloat())
        indicatorMarginLeft = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_margin_left, dp2px(0f).toFloat())
        indicatorMarginTop = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_margin_top, dp2px(if (mIndicatorStyle == STYLE_BLOCK) 7f else 0.toFloat()).toFloat())
        indicatorMarginRight = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_margin_right, dp2px(0f).toFloat())
        indicatorMarginBottom = ta.getDimension(R.styleable.base_TTabLayout_base_indicator_margin_bottom, dp2px(if (mIndicatorStyle == STYLE_BLOCK) 7f else 0.toFloat()).toFloat())
        isIndicatorAnimEnable = ta.getBoolean(R.styleable.base_TTabLayout_base_indicator_anim_enable, true)
        isIndicatorBounceEnable = ta.getBoolean(R.styleable.base_TTabLayout_base_indicator_bounce_enable, true)
        indicatorAnimDuration = ta.getInt(R.styleable.base_TTabLayout_base_indicator_anim_duration, -1).toLong()
        mIndicatorGravity = ta.getInt(R.styleable.base_TTabLayout_base_indicator_gravity, Gravity.BOTTOM)
        mUnderlineColor = ta.getColor(R.styleable.base_TTabLayout_base_underline_color, Color.parseColor("#ffffff"))
        mUnderlineHeight = ta.getDimension(R.styleable.base_TTabLayout_base_underline_height, dp2px(0f).toFloat())
        mUnderlineGravity = ta.getInt(R.styleable.base_TTabLayout_base_underline_gravity, Gravity.BOTTOM)
        mDividerColor = ta.getColor(R.styleable.base_TTabLayout_base_divider_color, Color.parseColor("#ffffff"))
        mDividerWidth = ta.getDimension(R.styleable.base_TTabLayout_base_divider_width, dp2px(0f).toFloat())
        mDividerPadding = ta.getDimension(R.styleable.base_TTabLayout_base_divider_padding, dp2px(12f).toFloat())
        mTextsize = ta.getDimension(R.styleable.base_TTabLayout_base_titlesize, sp2px(13f).toFloat())
        mTextSelectColor = ta.getColor(R.styleable.base_TTabLayout_base_textSelectColor, Color.parseColor("#ffffff"))
        mTextUnselectColor = ta.getColor(R.styleable.base_TTabLayout_base_textUnSelectColor, Color.parseColor("#AAffffff"))
        mTextBold = ta.getInt(R.styleable.base_TTabLayout_base_textBold, TEXT_BOLD_NONE)
        mTextAllCaps = ta.getBoolean(R.styleable.base_TTabLayout_base_textAllCaps, false)
        mIconVisible = ta.getBoolean(R.styleable.base_TTabLayout_base_iconVisible, true)
        mIconGravity = ta.getInt(R.styleable.base_TTabLayout_base_icon_gravity, Gravity.TOP)
        mIconWidth = ta.getDimension(R.styleable.base_TTabLayout_base_iconWidth, dp2px(0f).toFloat())
        mIconHeight = ta.getDimension(R.styleable.base_TTabLayout_base_iconHeight, dp2px(0f).toFloat())
        mIconMargin = ta.getDimension(R.styleable.base_TTabLayout_base_iconMargin, dp2px(2.5f).toFloat())
        mTabSpaceEqual = ta.getBoolean(R.styleable.base_TTabLayout_base_tab_space_equal, true)
        mTabWidth = ta.getDimension(R.styleable.base_TTabLayout_base_tab_width, dp2px(-1f).toFloat())
        mTabPadding = ta.getDimension(R.styleable.base_TTabLayout_base_tab_padding, if (mTabSpaceEqual || mTabWidth > 0) dp2px(0f).toFloat() else dp2px(10f).toFloat())
        ta.recycle()
    }
    ...
    fun notifyDataSetChanged() {
        ...
        for (i in 0 until tabCount) {
            tabView = when (mIconGravity) {
                Gravity.LEFT -> {
                    View.inflate(mContext, R.layout.base_layout_tab_left, null)
                }
                Gravity.RIGHT -> {
                    View.inflate(mContext, R.layout.base_layout_tab_right, null)
                }
                Gravity.BOTTOM -> {
                    View.inflate(mContext, R.layout.base_layout_tab_bottom, null)
                }
                else -> {
                    View.inflate(mContext, R.layout.base_layout_tab_top, null)
                }
            }
            tabView.tag = i
            addTab(i, tabView)
